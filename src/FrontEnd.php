...
class FrontEnd
{
    ...
    public function view(string $Page = ''): void
    {
        ...
        /** Pages path. */
        $this->PagesPath = __DIR__ . DIRECTORY_SEPARATOR . 'pages' . DIRECTORY_SEPARATOR;
        ...
        $FE = [
            /** "Generated by ..." line. */
            'GeneratedBy' => sprintf(
                $this->Loader->L10N->getString('label.Generated by %s'),
        ...
        if ($this->Loader->Configuration['core']['maintenance_mode']) {
            $FE['Warnings'][] = $this->Loader->L10N->getString('warning.Maintenance mode is enabled');
        }
        ...
        if ($FE['FormTarget'] === 'login') {
            ...
            if (!empty($_POST['username']) && empty($_POST['password'])) {
                ...
                $FE['state_msg'] = $this->Loader->L10N->getString('response.Username field empty');
            } elseif (!empty($_POST['username']) && !empty($_POST['password'])) {
                ...
                if (isset(
                    ...
                    $TryUser = $_POST['username'];
                    $FE['state_msg'] = $this->Loader->L10N->getString('response.Username doesn_t exist');
                }
            }
        ...
        if ($Page === '') {
            ...
            if (empty($RemoteYAMLphpMussel)) {
                ...
                $FE['info_phpmussel_stable'] = $this->Loader->L10N->getString('response.Error');

                /** phpMussel latest unstable. */
                $FE['info_phpmussel_unstable'] = $this->Loader->L10N->getString('response.Error');

                /** phpMussel branch latest stable. */
                $FE['info_phpmussel_branch'] = $this->Loader->L10N->getString('response.Error');
            } else {
                ...
                $FE['info_phpmussel_stable'] = empty($RemoteYAMLphpMusselArray['Stable']) ?
                    $this->Loader->L10N->getString('response.Error') : $RemoteYAMLphpMusselArray['Stable'];

                /** phpMussel latest unstable. */
                $FE['info_phpmussel_unstable'] = empty($RemoteYAMLphpMusselArray['Unstable']) ?
                    $this->Loader->L10N->getString('response.Error') : $RemoteYAMLphpMusselArray['Unstable'];
                ...
                if ($ThisBranch = substr($FE['ScriptVersion'], 0, strpos($FE['ScriptVersion'], '.') ?: 0)) {
                    ...
                    $FE['info_phpmussel_branch'] = empty($RemoteYAMLphpMusselArray['Branch'][$ThisBranch]['Latest']) ?
                        $this->Loader->L10N->getString('response.Error') : $RemoteYAMLphpMusselArray['Branch'][$ThisBranch]['Latest'];
                } else {
            ...
            if (empty($RemoteYamlPHP)) {
                /** PHP latest stable. */
                $FE['info_php_stable'] = $this->Loader->L10N->getString('response.Error');

                /** PHP latest unstable. */
                $FE['info_php_unstable'] = $this->Loader->L10N->getString('response.Error');
            ...
            foreach ([
                ...
                if (extension_loaded($ThisExtension['Lib'])) {
                    ...
                    $ThisResponse = $this->Loader->L10N->getString('response._Yes') . ' (' . $ExtensionVersion . ')';
                    ...
                    $FE['ExtensionsCopyData'] .= $this->ltrInRtf(
                        sprintf('- %s➡%s\n', $ThisExtension['Name'], $this->Loader->L10N->getString('response._No'))
                    );
                    $ThisResponse = '<span class="txtRd">' . $this->Loader->L10N->getString('response._No') . '</span>';
                }
        ...
        foreach ($List as $Item => $List) {
            ...
            if (!empty($_POST['DeleteAll'])) {
                ...
                $FE['state_msg'] .= '<code>' . $DeleteMe . '</code> ' . $this->Loader->L10N->getString(
                    unlink($this->Loader->QuarantinePath . $DeleteMe) ? 'response.File successfully deleted' : 'response.Failed to delete'
                ) . '<br />';
                continue;
            }
    ...
    private function signatureInformationHandler(string &$InfoRows, string &$SigInfoMenuOptions): void
    {
        ...
        if (!$this->Loader->loadShorthandData()) {
            $InfoRows = '<span class="s">' . $this->Loader->L10N->getString('response.Error') . '</span>';
            return;
        }
