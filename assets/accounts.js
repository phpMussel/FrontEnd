window['phpmussel-form-target']='accounts';
function acc(e,d,i,t){var o=function(e){w('stateMsg',e)},a=function(){w('stateMsg','{Loading}')};window.username=document.getElementById(e).value,window.password=document.getElementById(d).value,window.do=document.getElementById(t).value,'delete-account'==window.do&&$('POST','',['phpmussel-form-target','username','password','do'],a,function(e){w('stateMsg',e),document.getElementById('q1'+i).classList.add('fmDelete'),document.getElementById('q2'+i).classList.add('fmDelete'),document.getElementById('q3'+i).classList.add('fmDelete'),document.getElementById('q4'+i).classList.add('fmDelete')},o),'update-password'==window.do&&$('POST','',['phpmussel-form-target','username','password','do'],a,o,o)}
function calculatePasswordEntropy(t,g=0){const o=t.length;if(0===o)return g?'none':'';const n={};for(let r=0;r<o;r++){const o=t.charAt(r);n[o]=(n[o]||0)+1}let r=0;for(const t in n)if(Object.prototype.hasOwnProperty.call(n,t)){const c=n[t]/o;r+=Math.abs(c*Math.log2(c))}let c=o<8?0:o/8-1;t=r+c+(((/[a-z]/.test(t))&&(/[A-Z]/.test(t))&&(/[0-9]/.test(t))&&(/[^A-Za-z0-9]/.test(t)))*3)-((/[0-9]/.test(t))&&!(/[^0-9]/.test(t)));if (g){t=t<=0?0:t>=100?100:t*11.111;return 'linear-gradient(90deg,rgba(0,255,0,0.5) 0% '+t+'%,rgba(255,0,0,0.5) '+t+'%)'}else{return t<4?'<span class=\u{22}txtRd\u{22}>{PasswordStrengthLow}</span>':t>6.5?'<span class=\u{22}txtGn\u{22}>{PasswordStrengthHigh}</span>':'<span class=\u{22}txtOe\u{22}>{PasswordStrengthMedium}</span>'}}
